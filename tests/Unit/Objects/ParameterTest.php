...
it('requires path parameters to be required', function (): void {
    $param = Parameter::path('id', Schema::string());

    expect($param->toArray())->toBe([
        'name' => 'id',
        'in' => 'path',
        'required' => true,
        'schema' => ['type' => 'string'],
...
it('builds a query parameter', function (): void {
    $param = Parameter::query('search', Schema::string())
...
it('builds a header parameter', function (): void {
    $param = Parameter::header('X-Trace', Schema::string())->required(true);
...
it('builds a cookie parameter', function (): void {
    $param = Parameter::cookie('session', Schema::string());
...
it('emits style/explode/allowReserved/example', function (): void {
    $param = Parameter::query('tags', Schema::array())
        ->style('form')
        ->explode(true)
        ->allowReserved(true)
        ->example(['a', 'b']);
