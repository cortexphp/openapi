...
`Operation::responses()` accepts `Response` objects only and keys them from `Response::getStatusCode()` (`200`, `404`, `default`, …). To attach a `$ref` to a reusable response, use `->response()` with an explicit status key — `Response::ref()` returns a `Reference`, which cannot be passed to `responses()`:
...
Operation::get()
    ...
    ->response(401, Reference::response('Unauthorized'));
```

`responses()` replaces the map; call `response()` afterwards to add references without wiping the named constructors.
