...
Reference schemas anywhere with typed `Reference` shortcuts. `Reference` is an OpenAPI object — pass it into schema *slots* (`MediaType`, `Parameter`, `Header`, `Components::schema()`), not into `cortexphp/json-schema` composition methods such as `allOf()` / `items()` / `properties()`:
...
// Inside a JsonSchema (allOf/oneOf/anyOf) use ->ref() on a schema, not Reference
Schema::object()->allOf(
    Schema::object()->ref('#/components/schemas/BaseEntity'),
...
Reference them from operations using `Response::ref()` with `Operation::response()`. `Operation::responses()` only accepts `Response` objects — a `Reference` has no status code of its own:
